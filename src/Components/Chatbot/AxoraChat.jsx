          ...
          <div className="rounded-3xl border border-white/10 bg-black/30 p-4 shadow-[0_10px_40px_rgba(0,0,0,0.35)]">
            ...
            >
              <input
                ...
                disabled={loading}
                className="flex-1 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white placeholder-slate-500 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/40 disabled:opacity-50 disabled:cursor-not-allowed"
              />
              <button
                type="submit"
                disabled={loading || !input.trim()}
                className="flex items-center gap-2 rounded-2xl bg-blue-600 px-4 py-3 text-sm font-semibold text-white shadow-[0_15px_45px_rgba(59,130,246,0.45)] transition hover:bg-blue-500 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-blue-600"
              >
                <FaPaperPlane /> Send
              </button>
            </form>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {quickPrompts.map((prompt) => (
              <button
                ...
                disabled={loading}
                className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-left text-slate-200 hover:border-blue-400/50 hover:text-white transition disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {prompt}
              </button>
            ))}
